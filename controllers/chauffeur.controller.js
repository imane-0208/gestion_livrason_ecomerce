...
const livraisant = require("../models/livraison");
...
exports.acceptCommande = async(req,res)=>{
    ...
    //   une livraison de 140kg va être faite par voiture le 14/03/2022 de la ville de Safi vers la ville d'Agadir: Chauffeur Ali Ahmed
      logger.info(`une livraison de ${getLivraison[0].poids}kg va être faite par (voiture) le ${getLivraison[0].date_depart} de la ville ${getLivraison[0].ville_depart} vers la ville ${getLivraison[0].ville_arrive}: Chauffeur ${token.name_chauffeur} ${token.lastname_chauffeur}`)
      res.status(200).json({
        success: 1,
        chauf,
        getLivraison,
