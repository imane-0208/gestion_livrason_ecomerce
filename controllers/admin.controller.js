...
exports.login = async (req, res) => {
    try {
        ...
        const login_admin = await Admin.findOne({
            ...
            if (!login_admin || !(await bcrypt.compareSync(password, login_admin.password))) {
                return res.status(200).send({
                    err: 'email or password is incorrect',
                })
            } else {
                const id = login_admin._id;
                const name_admin = login_admin.name;
                const lastname_admin = login_admin.lastname;
                const email = login_admin.email;
                const role = "admin";
